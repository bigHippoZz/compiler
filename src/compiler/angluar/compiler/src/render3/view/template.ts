			...
			if (icus.size) {
				icus.forEach((refs: o.Expression[], key: string) => {
					if (refs.length === 1) {
						...
						const placeholder: string = wrapI18nPlaceholder(
		...
		if (inputs.length || outputs.length) {
			...
			if (attrExprs.length !== attrsLengthBeforeInputs) {
				attrExprs.splice(
					...
					0,
					o.literal(core.AttributeMarker.Bindings),
				);
			}
		}
				...
				this._bindingScope.set(
					...
					(scope: BindingScope, relativeLevel: number) => {
						...
						const refExpr = lhs.set(
							o.importExpr(R3.reference).callFn([o.literal(slot)]),
						);
