...
export class _ParseAST {
	...
	expectIdentifierOrKeyword(): string | null {
		...
		if (!n.isIdentifier() && !n.isKeyword()) {
			if (n.isPrivateIdentifier()) {
				...
				this.error(
					`Unexpected ${this.prettyPrintToken(n)}, expected identifier or keyword`,
				);
			}
			return null;
		}
		...
		if (this.consumeOptionalOperator("|")) {
			...
			do {
				...
				if (nameId !== null) {
					...
					fullSpanEnd =
						this.next.index !== -1 ? this.next.index : this.inputLength + this.offset;
	...
	error(message: string, index: number | null = null) {
		this.errors.push(
			new ParserError(message, this.input, this.locationText(index), this.location),
		);
		this.skip();
	}
